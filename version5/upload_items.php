...
<html>
      ...
      session_start();
       ...
       // echo $tags;
          $tags_break =  (explode(',',$tags,-1));
          $tag1 =" nmh mmm";// $tags_break[0];
          $tag2 = "hjmjm";//$//tags_break[1];
          $tag3 = "h mj ";// $ta//gs_break[2];
      ...
      //  echo $service;
       ...
       /* echo $item_title ; echo $category ; echo $description ; echo $seller_name; echo $seller_roll;*/
            $pass = 0;    
        if(isset($_POST['submitbtn'])){
            $result = mysqli_query($con, "SELECT MAX(itemID) FROM $service");
    ...
    if(isset($_POST["submitbtn"]) && isset($_FILES['fileToUpload']) && isset($_FILES['fileToUploadTwo']) && isset($_FILES['fileToUploadThr']) &&                    isset($_FILES['fileToUploadFour']))
            ...
            if(!strcmp($service, "sell") && $pass){
                 $price  =   test_input($_POST['price']);
                    $sql =   "INSERT INTO sell(`title`,`description`,`category`,`tag1`, `tag2`, `tag3`, `price`, `available` ,`owner`, `owner_contact`,             `pickup_location`,  `image1_src`, `image2_src`, `image3_src`,`image4_src`, `user`)
             ...
             else if(!strcmp($service, "auction")){
                  $startd = $_POST['startd'];   
                  $startt = $_POST['startt'];
                  $endd = $_POST['endd'];
                  $endt = $_POST['endt'];
              ...
              //    date_default_timezone_set();
                 $sql  = "INSERT INTO auction( item, description, category, imagesrc, imagesrc1, imagesrc2, imagesrc3, bprice, cprice, bnprice, start, end, user, status,bidder ) VALUES ('".$item_title."','".$description."','".$category."','".$target_file1."','".$target_file2."','".$target_file3."','".$target_file4."','".$bprice."','".$cprice."','".$bnprice."','".$start."','".$end."',$person,2, '".$person."');";
                                if ($con->query($sql) === TRUE) {
                                      echo "dataCreated sucesssfully";
                                      header("location:../auction/home.php");
