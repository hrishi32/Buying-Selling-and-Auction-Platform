...
<html>
    ...
    if(isset($_POST["submitbtn"]) && isset($_FILES['fileToUpload']) && isset($_FILES['fileToUploadTwo']) && isset($_FILES['fileToUploadThr']) &&                    isset($_FILES['fileToUploadFour']))
            ...
            if(!strcmp($service, "object") && $pass){
                ...
                $persons =  test_input($_POST['persons_sharing']);
                $price   =  test_input($_POST['original_price']);
                $sql =  "INSERT INTO share(`title`,`description`,`category`,`tag1` ,`tag2`, `tag3`, `price`, `available` ,`owner`, `owner_contact`,               `pickup_location`,  `image1_src`, `image2_src`, `image3_src`,`image4_src`, `user`, `persons`,`current_persons`)
                        VALUES('$item_title','$description','$category','$tag1','$tag2','$tag3','$price', 1, '$seller_name','$contact', '$address','$target_file1', '$target_file2','$target_file3','$target_file4','$person','$persons',0)";
                                    if ($db->query($sql) === TRUE) {
                                        echo "dataCreated sucesssfully";
                                        header("location:myuploads.php");
