<!DOCTYPE HTML>
	<html>
      ...
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
        ...
        $sql = "SELECT * FROM share WHERE id=$item_id AND user='$email'";
...
/*$arr = preg_split('/(?<=[0-9])(?=[a-z]+)/i',$image1);                                                      
       ...
       echo $arr[0];
        ...
        <header style="">
            ...
            <div id="navbar">
                <ul>
                    <li><div class="nav_divs"><a href="http://10.8.16.11/Version5/index.php">Home</a></div></li>
            ...
            <div id="right_info_containing_div" style="display:inline-block";>
                ...
                <div id="owner_info" style="padding:20px; width:50%; border:solid red 1px;">
                    ...
                    $sql="SELECT userid FROM user WHERE email = '".$_SESSION['email']."'";
                        ...
                        $sql="SELECT * FROM proposals WHERE item_id = ".$item_id." and status = 0;";
// echo "   query is    ".$sql;
                        $result = mysqli_query($db,$sql);
                        while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
                        {
                        	$current_usr = intval($row['user_id']);
                        	$current_sql = "SELECT username FROM user WHERE userid = $current_usr";
                        	// echo $current_sql;
                        	$current_result = mysqli_query($db,$current_sql);
                        	$current_row=mysqli_fetch_array($current_result,MYSQLI_ASSOC);
                        	echo "<table><tr><td>";
                        	echo $current_row['username'].": </td><td> ";
                            echo "<button type=button onclick=\"approve_user('".$item_id."','".$row['user_id']."')\" >Approve</button></td>";
                            echo "<td><button type=button onclick=\"deny_user('".$item_id."','".$row['user_id']."')\" >Deny</button></td>";
                            echo "</tr></table>";
                        }
                        ?>
                    ...
                    <div id="status"><b><!-- Person info will be listed here... --></b></div>
                </div>
                ...
                <h3 style="margin:0px;">Contributors</h3> 
                <?php
                	$sql="SELECT * FROM proposals WHERE item_id = ".$item_id." and status = 1;";
                	$result = mysqli_query($db,$sql);
                	if(!$result)
                	{
                		echo "NO contributers yet";
                	}
                	else
                	{
                		echo "<table>";
        ...
        <script>
             $(document).ready(function()
          {
             ...
             function approve_user(objid,usrid) {
            // window.location = "home.php" ;
                // document.getElementById("txtHint"+   objid).innerHTML = objid+" "+usrid;
                if (objid == 0) 
                {
                    document.getElementById("status").innerHTML = " udfishfiuuhfiuhf";
                    return;
                } 
                else 
                { 
                    ...
                    xmlhttp.open("GET","approve.php?userid="+usrid+"&shareid="+objid,true);
                    xmlhttp.send();
                }
            }
            ...
            function deny_user(objid,usrid) {
            // window.location = "home.php" ;
                // document.getElementById("txtHint"+   objid).innerHTML = objid+" "+usrid;
                if (objid == 0) 
                {
                ...
                { 
                    ...
                    xmlhttp.open("GET","deny.php?userid="+usrid+"&shareid="+objid,true);
                    xmlhttp.send();
                }
            }
